...
In my free time when working on open source projects, I often find myself typically working on build tools and CLI type projects.  These kind of projects, unlike the web apps I help build by day, typically take a combination of a user configuration file and some source code, and generate some sort of output from that, based on whatever the project is designed to do.  

For the project [Greenwood](https://www.greenwoodjs.io/), which is a site builder, the output of that project is going to be HTML, CSS and JavaScript, all based on the user's source code (e.g. markdown files).  To help ensure we don't break anything as we go, and taking influence from **webpack** and how they setup their tests, the majority of the project's test coverage is predominently made up of test "cases"; wherein fixtures and files are used to recreate what a users folder structure might look like while using the tool.  

In this way, given a user's config and content, I can ensure critical features are still working and validate behavior like:
